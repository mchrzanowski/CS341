...
def train_users(cd_steps, FEATURES, hidden_biases, ITEMS, SOFTMAX, users,
    user_to_items, visible_biases, weights):

  emissions = None
  for user in users:
    emission = train_user(cd_steps, FEATURES, hidden_biases, ITEMS, SOFTMAX, user,
    user_to_items, visible_biases, weights)
    if emissions is None:
      emissions = list(emission)
    else:
      for i, items in enumerate(emission):
        emissions[i] += items

  return emissions

def train_user(cd_steps, FEATURES, hidden_biases, ITEMS, SOFTMAX, user,
    user_to_items, visible_biases, weights):
...
incremental_CD, ITEMS, Momentum, number_of_users, SOFTMAX, weight_cost, weights, visual_bias_increment, visible_biases):
...
def train(FEATURES, hidden_biases, ITEMS, SOFTMAX, user_to_items,
    visible_biases, weights):
  ...
  while (nrmse < last_rmse) and loop_count < 80:
    ...
    for batch in xrange(number_of_batches):
      ...
      #pool.apply_async(train_users, args=(cd_steps, FEATURES, hidden_biases, ITEMS, SOFTMAX,
      #  user_keys[startIndex : endIndex],
      #  user_to_items, visible_biases, weights), callback=emissions.append
      #)

      emissions.append(train_users(cd_steps, FEATURES, hidden_biases, ITEMS, SOFTMAX,
        user_keys[startIndex : endIndex],
        user_to_items, visible_biases, weights))
    ...
    nrmse_update, ntrain_update = updateWeights(emissions, EpsilonHB, EpsilonW,
      ...
      incremental_CD, ITEMS, Momentum, len(user_keys), 
      SOFTMAX, weight_cost, weights,
    ...
    end_time = time.time()
    print "Loop: {0:2}\tNRMSE: {1:5}\tElapsed Time:{2} seconds".format(loop_count, nrmse, end_time - start_time)
...
def test(FEATURES, testing, hidden_biases, ITEMS, item_translation, labels, user_translation,
  ...
  measures = dict((r, numpy.zeros(3, dtype=numpy.int)) for r in xrange(SOFTMAX))
