...
public class ParallelRBM {
  ...
  private Map<String, Integer> itemTranslation;
  private Map<String, Integer> userTranslation;
  ...
  private ParallelRBMTrainer[] trainers;

  public ParallelRBM(int pNoOfFeatures) {
    NUM_FEATURES = pNoOfFeatures;
  }

  private void initializeTrainers() {
    // MAX_THREADS = Runtime.getRuntime().availableProcessors();
    this.trainers = new ParallelRBMTrainer[MAX_THREADS];
    for (int id = 0; id < MAX_THREADS; ++id) {
      trainers[id] = new ParallelRBMTrainer(id);
    }
  }
  ...
  /**
   * Train for a single pass over the data
   */
  private void trainSinglePass() {
    trainers[0].run();
    /*
    try {
   ...
   */
  private synchronized void stageWeightsUpdate(double[][][] cd_vh,
      double[][] cd_v, double[] cd_h, double nrmse) {
    add(this.cd_vh, cd_vh);
    add(this.cd_v, cd_v);
    add(this.cd_h, cd_h);
    this.nrmse += nrmse;
  }

  private void commitWeightsUpdate() {
  ...
  private class ParallelRBMTrainer implements Runnable {
    ...
    private double[][][] _cd_vh;
    private double[][] _cd_v;
    private double[] _cd_h;
    private double _nrmse;

    private int id;

    public ParallelRBMTrainer(int id) {
      this.id = id;
      ...
      _cd_vh = new double[NUM_ITEMS][NUM_LABELS][NUM_FEATURES];
      _cd_v = new double[NUM_ITEMS][NUM_LABELS];
      _cd_h = new double[NUM_FEATURES];
      _nrmse = 0;
    }

    public void run() {
      // train user if userId % id == 0
      for (int userId = 0; userId < NUM_USERS; ++userId) {
        trainSingleUser(userId);
      }
      // stage update in ParallelRBM
      stageWeightsUpdate(_cd_vh, _cd_v, _cd_h, _nrmse);
    }
    ...
    private void accumulatePositiveCD(int[] data, double[] poshidact) {
      ...
      // compute cd_h
      for (int feature = 0; feature < NUM_FEATURES; ++feature) {
        _cd_h[feature] += poshidact[feature];
      }
    }
    ...
    private void accumulateNegativeCD(int[] data, double[] neghidact,
      ...
      for (int idx = 0; idx < data.length; ++idx) {
        ...
        // compute cd_v
        _cd_v[item][label] -= negvisact[item][label];
      }
      // compute cd_h
      for (int feature = 0; feature < NUM_FEATURES; ++feature) {
        _cd_h[feature] -= neghidact[feature];
      }
    }
    ...
    private void computeHiddenGivenVisible(int[] data, double[] result) {
      ...
      for (int feature = 0; feature < NUM_FEATURES; ++feature) {
        result[feature] = 1.0 / (1.0 + Math
            .exp(-result[feature] - w_h[feature]));
      }
    }
    ...
    private void computeVisibleGivenHidden(int[] data, double[] hidden,
      ...
      for (int idx = 0; idx < data.length; ++idx) {
        ...
        for (int label = 0; label < NUM_LABELS; ++label) {
          double v = 1. / (1 + Math
              .exp(-result[item][label] - w_v[item][label]));
          result[item][label] = v;
          normalizer += v;
        }
        // normalize
  ...
  private void parseTrainingFile(String filename) {
    this.itemTranslation = new HashMap<String, Integer>();
    this.userTranslation = new HashMap<String, Integer>();
  ...
  public static void main(String[] args) {
    int numFeatures = 100;
    String trainingFileLocation = "./data/good_training";
    ...
    long startTime = System.currentTimeMillis();
    ParallelRBM rbm = new ParallelRBM(numFeatures);
